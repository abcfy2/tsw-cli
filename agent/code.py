...
class Config(BaseModel):
    ...
    mode: Literal["explanation", "tutorial", "review"] = Field(
...
def pack_code_into_markdown(c: Config):
    ...
    if not exist(repo_file):
        ...
        if path.isdir(c.repo):
            processor = RepoProcessor(directory=c.repo, config=config)
        else:
...
code_reviewer_agent = Agent(
    ...
    model=Gemini(id=GEMINI_MODEL_ID),
    description=dedent("""\
        You are a good code reviewer and keen to find bugs and potential issues in the code.
        - Don't include any linting issues or code style problems because there are tools for that.
        - Focus on duplicate code, performance issues, security vulnerabilities, and potential bugs.
        - If the code doesn't follow the best practices, point them out.
        - If the code doesn't follow the conventions of the language, point them out.
        - Be highly organized.\
        """),
    instructions=[
        "You will be given a text including the whole repository code.",
        "Review the code and find any potential issues.",
    ],
    expected_output=dedent("""\
    A code review report in markdown format:

    # {Code Review Report for the Repository}

    ## Summary
    {Brief overview of main issues in the codebase}
    ...
    ## Best Practices or Language Conventions
    ...
    ## Conclusion
    {A brief conclusion of the review}
    {The overall quality of the codebase}
    {The potential improvements that can be made}
    {The Recommendations for the developer}
    ---
    Report generated by TSW-X
    Advanced Research Systems Division
    Date: {current_date}\
...
def explain_repo(config: str):
    ...
    if c.mode == "explanation":
        ...
        write(c.report, code_reviewer_agent.run(code_for_agent).content)
    else:
